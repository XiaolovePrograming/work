...
__device__ unsigned int bswap32(unsigned int x){
  ...
  out[3] = input[0];  out[2] = input[1];
  out[1] = input[2];  out[0] = input[3];
  return result;
}

__device__  unsigned long bswap64(unsigned long x){
  unsigned long result;
  unsigned char* out = (unsigned char *)&result;
  unsigned char* input = (unsigned char *)&x;

  out[7] = input[0];out[6] = input[1];
  out[5] = input[2];out[4] = input[3];
  out[3] = input[4];out[2] = input[5];
  out[1] = input[6];out[0] = input[7];
  return result;
}

__device__ W R_replace(W v,int n){
...
__device__ void sm3_compress(sm3_ctx *c){
    ...
    for(i=16;i<68;i++)  w[i]=P1(w[i-16]^w[i-9]^R(w[i-3],15))^R(w[i-13],7)^w[i- 6];
    ...
    F(64) {
      ...
      s1=R(s2+x[4]+R(t,i),7);
      s2^=s1;
      if(i<16) {  t1=F1(x[0],x[1],x[2])+x[3]+s2+(w[i]^w[i+4]);  t2=F1(x[4],x[5],x[6])+x[7]+s1+w[i]; } 
      else     {  t1=FF(x[0],x[1],x[2])+x[3]+s2+(w[i]^w[i+4]);  t2=GG(x[4],x[5],x[6])+x[7]+s1+w[i]; }
                  x[3]=x[2];x[2]=R(x[1],9);x[1]=x[0];x[0]=t1;   x[7]=x[6];x[6]=R(x[5],19);x[5]=x[4];x[4]=P0(t2); }
    F(8)c->s[i]^=x[i];
}
